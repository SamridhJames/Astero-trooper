...
pyglet.resource.path = [os.path.join(
    workingDir, '../images'), os.path.join(workingDir, '../sounds')]
...
def run(start_func, w=window.width, h=window.height, bg=(1, 1, 1), fs=False, tc=(0.5, 0.5, 0.5), tfn='Broadway'):
    ...
    start = start_func
    #background = image('laser.png')
    background = bg
    snl, sbl, fl, pl = score_now_label, score_best_label, fps_label, pause_label
    snl.color = sbl.color = fl.color = pl.color = int(
        255*tc[0]), int(255*tc[1]), int(255*tc[2]), 255

...
def enemy(e):
    ...
    if e.state == 4 and random.random() < 0.01:
        create_object(homing_missile_enemy, image_missile_e,
                      0.05, e.x, e.y, -0.01, 0)
...
def player(p):
    ...
    for e in group(enemy, bullet_enemy, homing_missile_enemy, dropped_option):
        ...
        if d < e.sx:
            if hasattr(e, 'isOption') and e.isOption and len(options) < 3:
                sound_treasure.play()
                e.life = 0
                options.append(create_object(
                    option, image_option, 0.06, -0.9, 0))
                parent = p
    ...
    if key(Z) and p.time <= 0:
        ...
        sound_missile.play()
        for o in group(player, option):
            create_object(homing_missile, image_missile,
                          0.08, o.x, o.y, 0.03, life=10)
            p.time = 30
...
def stage(s):
    ...
    if random.random() < 0.01 + 0.0005 * score():
        ...
        create_object(enemy, enemy_image, size, x, y, -0.005,
                      life=enemy_life, state=enemy_type)

    if random.random() < 0.001 and len(options) < 3:
        size = 0.06
        create_object(dropped_option, image_option, size, SW+size,
                      random.uniform(-SH+size, SH-size), -0.005, isOption=True)
...
def start():
    ...
    p = create_object(player, image_player, 0.09, -0.9, 0)
...
run(start, 1280, 720, (0, 0, 0), False, (1, 1, 1))
